...
{
  PCLASSINFO(PBaseString, PObject)
  public:
    /* Type of conversion to make in the conversion constructors.
     */
    enum ConversionType {
      // deprecated  Pascal,   // Data is a length byte followed by characters.
      // deprecated  Basic,    // Data is two length bytes followed by characters.
      Literal = 2,  // Data is C language style string with \\ escape codes.
    ...
    template<typename int_type, class string_type>
    struct IntConversion
    {
      static void Psigned2string(string_type & s, int base, typename int_type v); 
      static void Punsigned2string(string_type & s, int base, typename int_type v);
    };

    template<typename char_type, class base_class>
    struct PStdStringTraits
    {
      static void Pconstruct(base_class & s);

      static void Passign(base_class & s, const char * ch);
      static void Passign(base_class & s, const wchar_t * ch);

      static void Passign(base_class & s, const char * ch, size_t len);
      static void Passign(base_class & s, const wchar_t * ch, size_t len);

      static void Pvsprintf(base_class & s, const char_type * fmt, va_list arg);

      static void Ptranslate_escapes(base_class & s, const char_type * src);

      static PINDEX Phash(const base_class & fmt);

      static void Preadfrom(base_class & s, istream & strm);

      static void Pprinton(const base_class & s, ostream & strm);

      static void Pminimise(base_class & str)
      { 
        const char_type * ptr = str.c_str();
        while (*ptr) 
          ptr++; 
        str.resize(ptr - str.c_str()); 
      }

      template <typename int_type>
      static void Psigned2string(base_class & s, int base, typename int_type v)   
      { IntConversion<typename int_type, base_class>::Psigned2string(s, base, v); }

      template <typename int_type>
      static void Punsigned2string(base_class & s, int base, typename int_type v) 
      { IntConversion<typename int_type, base_class>::Punsigned2string(s, base, v); };
    };
};


template <typename char_type, typename unsigned_char_type, typename base_class>
class PStdStringClass : public PBaseString, public base_class
{
  //PCLASSINFO(PStdStringClass, PBaseString)
  public:
    typedef PStdStringTraits<char_type, typename base_class> PTraits;

    /**Construct an empty string. This will have one character in it which is
       the '\\0' character.
     */
    PStdStringClass()
    { PTraits::Pconstruct(*this); }

    /**Create a new reference to the specified string. The string memory is not
       copied, only the pointer to the data.
     */
    PStdStringClass(
      const PStdStringClass & str  ///< String to create new reference to.
    )
    { PTraits::Passign(*this, (const char_type *)str); }
    ...
    */
    ...
    operator const unsigned_char_type *() const
    { return (const unsigned_char_type *)c_str(); }

    char_type * m_theArray()
    { return c_str(); }

    const char_type * m_theArray() const
    { return c_str(); }

    /**Create a string from various types of normal C strings

       Note that functions specifying a length will allow a string with embedded '\\0'
    ...
    { PTraits::Passign(*this, &ch, 1); }

    PStdStringClass(
      const char * cstr 
    )
    { PTraits::Passign(*this, cstr); }

    PStdStringClass(
      const char * cstr,  ///< Pointer to a string of characters.
      PINDEX len          ///< Length of the string in bytes.
    )
    { PTraits::Passign(*this, cstr, len); }

    PStdStringClass(
      const std::string & str
    )
    { PTraits::Passign(*this, str); }

    PStdStringClass(
      const PCharArray & str 
    )
    { PTraits::Passign((const char *)str, str.GetSize()); }

    /**Create a string from various types of wide C strings
     ...
     */
    PStdStringClass(
      wchar_t ch    ///< Single character to initialise string.
    )
    { PTraits::Passign(*this, &ch, 1); }
    ...
    PStdStringClass(
      const wchar_t * cstr,  ///< Pointer to a string of characters.
      PINDEX len             ///< Length of the string in bytes.
    )
    { PTraits::Passign(*this, cstr, len); }
    ...
    PStdStringClass(
      const PWCharArray & str ///< UCS-2 null terminated string.
    )
    { PTraits::Passign((const wchar_t *)str, str.GetSize()); }
    ...
    { PTraits::Psigned2string<short>(*this, 10, n); }
    ...
    { PTraits::Punsigned2string<unsigned short>(*this, 10, n); }
    ...
    { PTraits::Psigned2string<int>(*this, 10, n); }
    ...
    { PTraits::Punsigned2string<unsigned int>(*this, 10, n); }
    ...
    { PTraits::Psigned2string<long>(*this, 10, n); }
    ...
    { PTraits::Punsigned2string<unsigned long>(*this, 10, n); }
    ...
    { PTraits::Psigned2string<PInt64>(*this, 10, n); }
    ...
    { PTraits::Punsigned2string<PUInt64>(*this, 10, n); }

    /* Contruct a new string converting from the specified data source into
       a string array.
     */
    PStdStringClass(
    ...
    { 
      ...
      switch (type) {
        case Signed :
          PTraits::Psigned2string<long>(*this, base, value);
          break;

        case Unsigned :
          PTraits::Punsigned2string<long>(*this, base, value); 
          break;
    ...
    )
    {
      switch (type) {
        case Decimal :
          PTraits::sprintf(*this, "%0.*f", (int)places, value);
          break;

        case Exponent :
          PTraits::sprintf(*this, "%0.*e", (int)places, value);
          break;
    ...
    PStdStringClass(
      ConversionType type,  ///< Type of data source for conversion.
      const char_type * str,    ///< String to convert.
    ...
    )
    {
      switch (type) {
        case Literal :
          ...
          PTraits::Ptranslate_escapes(*this, str);
  ...
  //@{
     ...
     */
    PStdStringClass & sprintf(
      const char_type * cfmt,   ///< C string for output format.
      ...                  ///< Extra parameters for #sprintf()# call.
    )
    { va_list args; va_start(args, cfmt); PTraits::Pvsprintf(*this, cfmt, args); return *this; }
    PStdStringClass & sprintf(
      const PStdStringClass & fmt, ///< String for output format.
      ...                        ///< Extra parameters for #sprintf()# call.
    )
    { va_list args; va_start(args, fmt); PTraits::Pvsprintf(*this, (const Char *)fmt, args); return *this; }

    PStdStringClass & vsprintf(
      const char_type * cfmt,   ///< C string for output format.
      va_list args         ///< Extra parameters for #sprintf()# call.
    )
    { PTraits::Pvsprintf(*this, cfmt, args); return *this; }
    PStdStringClass & vsprintf(
      const PStdStringClass & fmt, ///< String for output format.
      va_list args               ///< Extra parameters for #sprintf()# call.
    )
    { PTraits::Pvsprintf(*this, (const char_type *)fmt, args); return *this; }
  ...
  //@{
    ...
    { PTraits::Pminimise(*this); return true; }
    ...
    /**Determine if the string is empty. This is semantically to 
       #GetLength()#  == 0.
    ...
    */
    ...
    /**Make the current string empty
      */
    virtual PStdStringClass & MakeEmpty()
    { clear(); return *this; }

    /**Return an empty string.
      */
    static PStdStringClass Empty()
    { return PStdStringClass(); }
  //@}
     ...
     */
    PStdStringClass & operator=(
      const char_type * cstr  ///< C string to assign.
    )
    { *this = cstr; return *this; }
    PStdStringClass & operator=(
      char char_type          ///< Character to assign.
    )
    { *this = ch; return *this; }
  ...
  //@{
    ...
    { return new PStdStringClass(*this); }
    ...
    virtual Comparison Compare(
      const PStdStringClass & str
    ) const
    { 
      if (((const base_class &)*this) < str.c_str())
        return PObject::LessThan;
      else if (((const base_class &)*this) > str.c_str())
        return PObject::GreaterThan;
      else
        return PObject::EqualTo;
    }
    virtual Comparison Compare(
      const PObject &    ///< Other PString to compare against.
    ) const
    { 
      return PObject::LessThan;
    }
    ...
    { return PTraits::Phash(*this); }
  //@}
};


template <typename Char>
struct PStdCaselessTraits : public char_traits<Char>
{
...
/**Produce formatted output as a string. This is identical to the standard
   ...
   reference to the current string object.
 */
PStdString PStd_psprintf(
  const char * cfmt,   ///< C string for output format.
  ...                  ///< Extra parameters for #sprintf()# call.
);

PStdString PStd_psprintf(
  const PStdString & fmt, ///< C string for output format.
  ...                    ///< Extra parameters for #sprintf()# call.
);

PStdString PStd_pvsprintf(
  const char * cfmt,   ///< C string for output format.
  va_list args         ///< Extra parameters for #sprintf()# call.
);

PStdString PStd_pvsprintf(
  const PStdString & fmt, ///< String for output format.
  va_list args          ///< Extra parameters for #sprintf()# call.
);


PStdWideString PStd_pswprintf(
  const char * cfmt,   ///< C string for output format.
  ...                  ///< Extra parameters for #sprintf()# call.
);

PStdWideString PStd_pswprintf(
  const PStdWideString & fmt, ///< C string for output format.
  ...                    ///< Extra parameters for #sprintf()# call.
);

PStdWideString PStd_pvswprintf(
  const char * cfmt,   ///< C string for output format.
  va_list args         ///< Extra parameters for #sprintf()# call.
);

PStdWideString PStd_pvswprintf(
  const PStdWideString & fmt, ///< String for output format.
  va_list args          ///< Extra parameters for #sprintf()# call.
);
