                                   ...
                                   PBoolean isGET)
{
  ...
  if (CheckAuthority(server, *request, connectInfo)) {
    ...
    if (!LoadHeaders(*request)) 
      ...
      hitCount++;
      retVal = OnGETData(connectInfo, *request);
    }
  }
...
{
  return OnGETData(connectInfo, request);
}

PBoolean PHTTPResource::OnGETData(const PHTTPConnectionInfo & /*connectInfo*/, PHTTPRequest & request)
{
  SendData(request);
