...
template <class T, class C> C * p_unsigned2string(T value, int base, C * str)
{
  if (value >= (T)base)
    str = p_unsigned2string<T, C>(value/(T)base, base, str);
  value %= base;
  if (value < 10)
    *str = (C)(value + '0');
  else
...
template <class T, typename C> C * p_signed2string(T value, int base, C * str)
{
  if (value >= 0)
    return p_unsigned2string<T, C>(value, base, str);

  *str = '-';
  return p_unsigned2string<T, C>(-value, base, str+1);
}
...
{
  p_signed2string<int, char>(n, 10, theArray);
  MakeMinimumSize();
}
...
{
  p_unsigned2string<unsigned int, char>(n, 10, theArray);
  MakeMinimumSize();
}
...
{
  p_signed2string<long, char>(n, 10, theArray);
  MakeMinimumSize();
}
...
{
  p_unsigned2string<unsigned long, char>(n, 10, theArray);
  MakeMinimumSize();
}
...
{
  p_signed2string<PInt64, char>(n, 10, theArray);
  MakeMinimumSize();
}
...
{
  p_unsigned2string<PUInt64, char>(n, 10, theArray);
  MakeMinimumSize();
}
  ...
  : PCharArray(sizeof(long)*3+1)
{
  ...
  switch (type) {
    case Signed :
      p_signed2string<long, char>(value, base, theArray);
      break;

    case Unsigned :
      p_unsigned2string<unsigned long, char>(value, base, theArray);
      break;
...
static int GetBaseSize(int base)
{
  switch (base) {
    case 2:
      return 8;
...
static int PtranslateHex(char_type x)
{
  if (x >= 'a')
    return x - 'a' + 10;
...
template <typename char_type>
static void TranslateEscapes(char_type * dst, const char_type * src)
{
  if (*src == '"')
    src++;
...
#define DECLARE_INT_CONVERSIONS(signed_type, unsigned_type) \
template<> void PBaseString::IntConversion<signed_type, std::string>::Psigned2string(std::string & str, int base, signed_type v) \
{ \
  int s = GetBaseSize(base); \
  if (s > 0) { \
    str.resize(sizeof(signed_type)*s+1); \
    p_signed2string<signed_type, char>(v, base, &str[0]); \
  } \
} \
template<> void PBaseString::IntConversion<unsigned_type, std::string>::Punsigned2string(std::string & str, int base, unsigned_type v) \
{ \
  int s = GetBaseSize(base); \
  if (s > 0) { \
    str.resize(sizeof(unsigned_type)*3+1); \
    p_unsigned2string<unsigned_type, char>(v, base, &str[0]); \
  } \
} \
template<> void PBaseString::IntConversion<signed_type, std::wstring>::Psigned2string(std::wstring & str, int base, signed_type v) \
{ \
  int s = GetBaseSize(base); \
  if (s > 0) { \
    str.resize(sizeof(signed_type)*3+1); \
    p_signed2string<signed_type, wchar_t>(v, base, &str[0]); \
  } \
} \
template<> void PBaseString::IntConversion<unsigned_type, std::wstring>::Punsigned2string(std::wstring & str, int base, unsigned_type v) \
{ \
  int s = GetBaseSize(base); \
  if (s > 0) { \
    str.resize(sizeof(unsigned_type)*3+1); \
    p_unsigned2string<unsigned_type, wchar_t>(v, base, &str[0]); \
  } \
} \

#define DECLARE_STRING_FUNCTIONS(string_type, prefix, char_type, base_string_type) \
string_type PStd_ps##prefix##printf(const char * cfmt, ...)                 { string_type str; va_list args; va_start(args, cfmt); str.vsprintf(cfmt, args); return str; } \
string_type PStd_ps##prefix##printf(const string_type & fmt, ...)           { string_type str; va_list args; va_start(args, fmt); str.vsprintf((const char_type *)fmt, args); return str; } \
string_type PStd_psv##prefix##printf(const char * cfmt, va_list args)       { string_type str; return str.vsprintf(cfmt, args); } \
string_type PStd_psv##prefix##printf(const string_type & fmt, va_list args) { string_type str; return str.vsprintf(fmt, args); } \
template<> void string_type::PTraits::Ptranslate_escapes(base_string_type & s, const char_type * src) { TranslateEscapes<char_type>(&s[0], src); } \
template<> void string_type::PTraits::Pvsprintf(base_string_type & s, const char_type * fmt, va_list arg) \
{ \
  PINDEX len = s.length(); \
  PINDEX providedSpace = 0; \
  int requiredSpace; \
  do { \
    providedSpace += 1000; \
    s.resize(providedSpace+len); \
    requiredSpace = _vsn##prefix##printf(&s[0], providedSpace, fmt, arg); \
  } while (requiredSpace == -1 || requiredSpace >= providedSpace); \
  Pminimise(s); \
} \
template<> PINDEX string_type::PTraits::Phash(const base_string_type & fmt) \
{ \
  PINDEX hash = 0; \
  const char_type * p = fmt.c_str(); \
  for (PINDEX i = 0; i < 8 && p[i] != 0; i++) { \
    hash = (hash << 5) ^ tolower(p[i] & 0xff) ^ hash; \
    ++p; \
  } \
  return PABSINDEX(hash)%127; \
} \
...
template<> void PStdString::PTraits::Pconstruct(std::string & /*str*/)                              { }

template<> void PStdString::PTraits::Passign(std::string & str, const char * cstr)               { str.assign(cstr); }
template<> void PStdString::PTraits::Passign(std::string & /*str*/, const wchar_t * /*str*/)     {  }

template<> void PStdString::PTraits::Passign(std::string & str, const char * cstr, size_t len)   { str.assign(cstr, len); }
template<> void PStdString::PTraits::Passign(std::string & /*str*/ , const wchar_t * /*cstr*/, size_t /*len*/) {  }

template<> void PStdString::PTraits::Preadfrom(std::string & /*str*/ , istream & /*strm*/ )                { }
template<> void PStdString::PTraits::Pprinton(const std::string & str, ostream & strm)
{
  PINDEX width = (int)strm.width();
  if (width > (PINDEX)str.length())
    width -= str.length();
  else
    width = 0;
...
DECLARE_STRING_FUNCTIONS(PStdString, , char, std::string)
...
template<> void PStdWideString::PTraits::Pconstruct(std::wstring & /*str*/)                              { }

template<> void PStdWideString::PTraits::Passign(std::wstring & /* str */, const char * /* cstr */)   { }
template<> void PStdWideString::PTraits::Passign(std::wstring & str, const wchar_t * cstr)            { str.assign(cstr); }

template<> void PStdWideString::PTraits::Passign(std::wstring & /* str */, const char * /* cstr */, size_t /* len */)   {  }
template<> void PStdWideString::PTraits::Passign(std::wstring & str , const wchar_t * cstr, size_t len) { str.assign(cstr, len);  }

template<> void PStdWideString::PTraits::Preadfrom(std::wstring & /*str*/, istream & /*strm*/ )                       { }
template<> void PStdWideString::PTraits::Pprinton(const std::wstring & /*str*/ , ostream & /*strm*/)                  { }

DECLARE_STRING_FUNCTIONS(PStdWideString, w, wchar_t, std::wstring)
...
DECLARE_INT_CONVERSIONS(short,  unsigned short)
DECLARE_INT_CONVERSIONS(int,    unsigned int)
DECLARE_INT_CONVERSIONS(long,   unsigned long)
DECLARE_INT_CONVERSIONS(PInt64, PUInt64)
