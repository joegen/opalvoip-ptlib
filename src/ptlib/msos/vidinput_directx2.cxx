...
PBoolean BindFilter(const PString & DeviceName, IBaseFilter **pFilter)
{
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tUnable to Enumerate system devices");
    return FALSE;
  }
  ...
  hr = pCreateDevEnum->CreateClassEnumerator(CLSID_VideoInputDeviceCategory,
    &pEm, 0);
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tUnable to Enumerate Video Input devices");
    return FALSE;
  }
  ...
  PTRACE(5,"DShow\tDevice " << DeviceName << " bound to Filter");
  return TRUE;
}
...
public:
  ...
  bool GetCurrentBuffer(BYTE & buffer, const long & expectedSize) 
  {
    ...
    if (bufferSize == 0) { 
      PTRACE(2,"DShow\tGRAB FAILURE! Current Buffer Empty");
      return FALSE;
    }

    if (bufferSize != expectedSize) {
      PTRACE(2,"DShow\tError: Current Buffer wrong size " << bufferSize << " expecting " << expectedSize);
      return FALSE;
    }
...
PStringArray PVideoInputDevice_DirectShow2::GetInputDeviceNames()
{
  ...
  PTRACE(4,"DShow\tDevices enumerated: " << setfill(',') << Names);
  return Names;
}
...
{ 
  ...
  if(FAILED(hr)) {
    PTRACE(2,"DShow\tCannot initialise DirectShow GraphBuilder");
    SAFE_RELEASE(t_pGB);
    return FALSE;
  }
...
#endif
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot initialise DirectShow capture graph builder");
    SAFE_RELEASE(t_pCap);
    SAFE_RELEASE(t_pGB);
    return FALSE;
  }
  ...
  if(!BindFilter(deviceName, &t_pDF)) {
    PTRACE(2,"DShow\tCannot bind webcam " << deviceName << " to filter");
    SAFE_RELEASE(t_pDF);
    SAFE_RELEASE(t_pCap);
    SAFE_RELEASE(t_pGB);
    return FALSE;
  ...
  if (SUCCEEDED(hr)) {
    PTRACE(5,"DShow\t" << deviceName << " is a video decoding device.");
    SAFE_RELEASE(t_pVD);
    SAFE_RELEASE(t_pDF);
    SAFE_RELEASE(t_pCap);
    SAFE_RELEASE(t_pGB);
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot add video capture filter "  << deviceName );
    SAFE_RELEASE(t_pDF);
    SAFE_RELEASE(t_pCap);
    SAFE_RELEASE(t_pGB);
    return FALSE;
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCould not enumerate " << deviceName << " Output pins " << FailedMSG(hr));
    SAFE_RELEASE(t_pDF);
    SAFE_RELEASE(t_pCap);
    SAFE_RELEASE(t_pGB);
    return FALSE;
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tUnable to find WebCam "  << deviceName << " Output pin " << FailedMSG(hr));
  ...
  if (FAILED(hr)) {
    PTRACE(4,"DShow\tWebCam " << deviceName << " does not support Camera Controls.");
    goto endcontroltest;
  } 
...
endcontroltest:
  ...
  if (FAILED(hr)) {
    ...
    SAFE_RELEASE(t_pCap);
    SAFE_RELEASE(t_pGB);
    PTRACE(2,"DShow\tCannot Query webcam Configuration " << deviceName );
    return FALSE;
  }
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot retrieve number of webcam capabilities " << deviceName );
    SAFE_RELEASE(pConfig);
    SAFE_RELEASE(t_pCamOutPin);
    SAFE_RELEASE(t_pCC);
    SAFE_RELEASE(t_pDF);
...
PBoolean PVideoInputDevice_DirectShow2::SetVideoFormat(IPin * pin) {
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot Query webcam Configuration");
    return FALSE;
  }
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot retrieve number of webcam capabilities ");
    SAFE_RELEASE(pConfig);
    return FALSE;
  }
  ...
  )
{
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot add video capture filter");
    return FALSE;
  }
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCould not enumerate Output pins " << FailedMSG(hr));
    return FALSE;
  }
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tUnable to find WebCam Output pin " << FailedMSG(hr));
    return FALSE;
  }
...
endcontroltest:
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot initialise Sample Grabber");
    return FALSE;
  }

  hr=m_pGB->AddFilter(m_pGrab, L"Sample Grabber");
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot add grabber filter");
    return FALSE;
  }

  hr=m_pGrab->QueryInterface(IID_ISampleGrabber, (void **)&m_pGrabber);	
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot find Grabber Interface");
    return FALSE;
  }
  ...
  if FAILED(hr) {
    PTRACE(2,"DShow\tFailed to Set Grabber Media Type : " << FailedMSG(hr));
    return FALSE;   
  }
  ...
  if FAILED(hr) {
    PTRACE(2,"DShow\tFailed to Set Grabber buffer samples : " << FailedMSG(hr));
    return FALSE;   
  }
  ...
  if FAILED(hr) {
    PTRACE(2,"DShow\tFailed to Set Grabber Callback : " << FailedMSG(hr));
    return FALSE;   
  }
  ...
  if FAILED(hr) {
    PTRACE(2,"DShow\tFailed to connect webcam to Grabber filter : " << FailedMSG(hr));
    return FALSE;   
  }
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot initialise NULL Renderer");
    return FALSE;
  }

  hr=m_pGB->AddFilter(m_pNull, L"NULL Output");
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot add NULL output filter");
    return FALSE;
  }
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot Null Output Filter to transform filter " <<  FailedMSG(hr));
    return FALSE;
  }  
  ...
  if (FAILED(hr)) {
    PTRACE(2,"DShow\tCannot access Media control interface");
    return FALSE;
  }
  ...
  PTRACE(4,"DShow\tDevice " << DeviceName << " open.");
...
PBoolean PVideoInputDevice_DirectShow2::Start()
{
  ...
  if((m_psCurrent == Paused) || (m_psCurrent == Stopped))
  {
    ...
    PTRACE(2,"DShow\tUnable to Start Video. " << FailedMSG(hr));
  }

  PTRACE(2,"DShow\tUnable to Start Video. " << m_psCurrent );
  return FALSE;
}
...
PBoolean PVideoInputDevice_DirectShow2::SetColourFormat(const PString & newColourFormat)
{
